...
class ICarl(IncrementalLearner):
    ...
    def _compute_loss(self, inputs, logits, targets, index):
        if self._old_model is None:
            ...
            new = F.pairwise_distance(new_feature[0], new_feature[0])
            old = F.pairwise_distance(old_feature[0], old_feature[0])
            ...
            """
            #####linear transform#################
            X = torch.cat((old.view(-1,1).cpu(),torch.ones(len(old),1)),1).to(self._device)
            a = torch.mm(torch.mm(torch.inverse(torch.mm(torch.t(X),X)),torch.t(X)),new.view(-1,1))
            pred_old = torch.mm(X,a)
            new = new.view(-1,1)
            
            self._graph_loss.append(F.l1_loss(new,pred_old).item())
            loss += 1*F.l1_loss(new,pred_old)
            """
            ##################################################            
            #self._graph_loss.append(F.l1_loss(new,old).item())
            #loss += (1+0.01*len(targets[0])/2)*F.l1_loss(new,old)
            g_loss = 0.001 * len(targets[0]) / 2 * F.mse_loss(new, old,reduction='sum')
            loss += g_loss
            g_loss = g_loss.item()
            self._graph_loss.append(g_loss)
            #print('MSE:',F.mse_loss(new,old,reduction = 'sum'))
            #print('l1:',F.l1_loss(new,old,reduction = 'sum'))
