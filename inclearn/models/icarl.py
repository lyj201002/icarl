...
class ICarl(IncrementalLearner):
    ...
    def _compute_loss(self, inputs, logits, targets, index):
        if self._old_model is None:
            ...
            if len(old_targets)==0:
                ...
                g_loss = self._lembda * self._graph(old, old_targets)  #change 1 to lembda
                #g_loss = (self._lembda+0.01*len(targets[0])/2) * self._graph(old, old_targets)
                loss +=  g_loss
                g_loss = g_loss.item()
            ...
            for i in range(len(new_feature)):
                for j in range(i+1,len(new_feature)):
                    if (i==0) & (j==1):
                        new = F.pairwise_distance(new_feature[i], new_feature[j])
                    else:
                        #Euclidean distance
                        new = torch.cat((new,F.pairwise_distance(new_feature[i],new_feature[j])))
                        #cosine similarity
            ...
            for i in range(len(old_feature)):
                for j in range(i+1,len(old_feature)):
                    if (i==0) & (j==1):
                        old = F.pairwise_distance(old_feature[i], old_feature[j])
                    else:
                        #Euclidean distance
                        old = torch.cat((old,F.pairwise_distance(old_feature[i],old_feature[j])))
                        #cosine similarity
                        #old = torch.cat((old,F.pairwise_distance(old_feature[i],old_feature[j])))
            ...
            if len(new_feature)<2:
                ...
                g_loss = self._lembda*F.l1_loss(new,old)        #change 1 to lembda
                g_loss = (self._lembda+0.01*len(targets[0])/2)*F.l1_loss(new,old)        
                #g_loss = 1*F.l1_loss(new,old) 
                loss += g_loss
                g_loss = g_loss.item()
            #self._graph_loss.append(g_loss)
            ############################################
    ...
    def _graph(self, data, label):
        new_feature = self._network.extract(data.to(self._network.device))
        ...
        old_MST = self._MST(old_feature)
        old_MST = old_MST.numpy()
        
        loss = 0
        
        # MST
        #for i in range(1,len(old_MST)):
            #x_TLx
            #loss += self._weight(old_feature[i],old_feature[old_MST[i]], dim=0) * self._Euclidean(new_feature[i],new_feature[old_MST[i]], dim=0)
            
            #|x_TLx - x_TLx|
            #loss += self._weight(old_feature[i],old_feature[old_MST[i]], dim=0) * F.l1_loss(self._Euclidean(new_feature[i],new_feature[old_MST[i]], dim=0) , self._Euclidean(old_feature[i],old_feature[old_MST[i]], dim=0))
        #all
        for i in range(len(new_feature)):
            for j in range(len(new_feature)):
                #loss += self._weight(old_feature[i],old_feature[j], dim=0) * self._Euclidean(new_feature[i],new_feature[j], dim=0)
                loss += self._weight(old_feature[i],old_feature[j], dim=0) * F.l1_loss(self._Euclidean(new_feature[i],new_feature[j], dim=0) , self._Euclidean(old_feature[i],old_feature[j], dim=0))
                #loss += 1 * F.l1_loss(self._Euclidean(new_feature[i],new_feature[j], dim=0) , self._Euclidean(old_feature[i],old_feature[j], dim=0))
                
        return loss / (len(new_feature)*len(new_feature))
    ...
    def _MST(self, data):
        ...
        MAX = 99999
        start = 0
        parent = torch.zeros(length, dtype=torch.int)
        visit = torch.zeros(length, dtype=torch.int)
        visit[start] = 1

        weight = self._weight(start, data, dim=1)
        
        for _ in range(length-1):
            temp_weight = self._weight(data[start], data, dim=1)
    ...
    def _weight(self, input1, input2, dim):
        return torch.exp(-0.5*self._Euclidean(input1, input2, dim))
    
    def _Euclidean(self, input1, input2,dim):
        return torch.sum(torch.pow(input1-input2,2),dim=dim)
