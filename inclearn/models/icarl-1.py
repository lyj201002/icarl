...
#                 print('loss.shape:', loss.shape)
                ...
                # calc the graph exemplar batch                
                idx_max = 0
                exemplar_batch = 20
                
                if self._data_transform_memory is not None:                    
                    # initialize
                    idx_max = self._data_transform_memory.shape[0] // exemplar_batch                   
#                     print('exemplar_batch:',exemplar_batch)
#                     print('self._data_transform_memory.shape[0]:',self._data_transform_memory.shape[0])
#                     print('idx_max:',idx_max)
                    for idx in range(idx_max):
                        ...
                        loss_2 = 0
                        feature_metric = None
                        self._optimizer_graph.zero_grad()
                        
                        # sample exemplar
                        if idx == 0:
                            choose_data = torch.tensor(self._data_transform_memory[0:exemplar_batch,:])
                            # get the exemplar from label
                            self.metric_3 = self.metric_2[0:exemplar_batch, 0:exemplar_batch]
                            # get the exemplar from old record
#                             self.metric_3 = self.metric[0:exemplar_batch, 0:exemplar_batch]
#                             print('self.metric_3:',self.metric_3)
#                             print('self.metric_3.shape:',self.metric_3.shape)
                        else:
                            choose_data = torch.tensor(
                                self._data_transform_memory[exemplar_batch * (idx-1):exemplar_batch * idx,:]
                            )
                            # get the exemplar from label
                            self.metric_3 = self.metric_2[exemplar_batch * (idx-1):exemplar_batch * idx, exemplar_batch * (idx-1):exemplar_batch * idx]
                            # get the exemplar from old record
#                             self.metric_3 = self.metric[exemplar_batch * (idx-1):exemplar_batch * idx, exemplar_batch * (idx-1):exemplar_batch * idx]
#                             print('self.metric_3:',self.metric_3)
#                             print('self.metric_3.shape:',self.metric_3.shape)

                        # get the exemplar in the feature space
                        features = self._network.extract(
                            choose_data.to(self._network.device)
                        )                   
                        # calc the metric
...
#                         print('feature_metric.shape:', feature_metric.shape)
                        ...
                        # calc the metric loss
                        first_part = self.metric_3.mul(torch.pow(feature_metric, 2))
                        second_part = (1 - self.metric_3).mul(torch.pow(torch.clamp(self.margin - feature_metric, min=0.0), 2))
    ...
    def build_examplars(self, inc_dataset):
        ...
        for class_idx in range(self._n_classes):
            ...
            for inputs_t, _ in loader:                 
                tmp.append(inputs_t.cpu().data.numpy())
        ...
        # use the label to construct the symmetric
